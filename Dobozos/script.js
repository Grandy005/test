...
elementTwo.addEventListener('mouseover', (event) => {
    elementTwo.style.backgroundColor = 'red';
});


elementTwo.addEventListener('mouseleave', (event)=> {
  elementTwo.style.backgroundColor = 'firebrick';
});
/*
3. doboz:
...
document.getElementById('element-three').addEventListener('dblclick', (event)=> {
  event.target.children[0].innerHTML = Math.floor(Math.random() * 20) + 1;
});
